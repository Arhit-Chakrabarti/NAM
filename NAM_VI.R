...
Run_NAM_VI <- function(X, Y, H = H, L = H, maxIter = maxIter, epsilon = epsilon,  warmstart = TRUE){
  ...
  for(iter in 1:maxIter){
    ...
    ###############################################################################################
    RHO_jk = Update_RHOjk_cpp(XI_jil = XI_jil,
                              X = t(X),
                              ml = mk_x,
                              tl = lambdak_x,
                              cl = nuk_x,
    ...
    ###############################################################################################
    # Eigth Update Theta_k^X  k = 1,..,H
    ###############################################################################################
    THETAl_X = Update_THETAl_X_cpp_mvt(X = t(X),
                                       RHO_jk = RHO_jk,
                                       m0 = m0_x,
                                       lambda0 = lambda0_x,
                                       nu0 = nu0_x,
    ...
    mk_x = THETAl_X["mk"]; mk_x = mk_x$mk
    lambdak_x = THETAl_X["lambdak"]; lambdak_x = lambdak_x$lambdak
    nuk_x = THETAl_X["nuk"]; nuk_x = nuk_x$nuk
    Wk_x =  THETAl_X["Wk"]; Wk_x = Wk_x$Wk 
    Sk_x =  THETAl_X["Sk_over_Nk"]; Sk_x = Sk_x$Sk_over_Nk
    Xbar =  THETAl_X["Xbar"]; Xbar = Xbar$Xbar
    Nk_x =  THETAl_X["Nk"]; Nk_x = Nk_x$Nk
    ...
    lCpl0_x  = log_Const_prod_gamma(D = q, nu = lambda0_x)
    LlogB0_x = H * logB_wish(W = W0_x , nu = lambda0_x, lCpl0_x);
    
    elbo9 = elbo_p_THETA_X(m0 = m0_x, t0 = lambda0_x, c0 = nu0_x,
                           D0 = W0_x, iD0 = iW0_x,
                           lCpl0 = lCpl0_x, LlogB0 = LlogB0_x,
                           mk = mk_x,
                           tk = lambdak_x,
                           ck = nuk_x, Dk = Wk_x)
    
    elbo10 = elbo_p_X(RHO_jk = RHO_jk,
                      mk = mk_x,
                      tk = lambdak_x,
                      ck = nuk_x,
                      Dk = Wk_x,
    ...
    elbo11 = elbo_q_THETA_Y(ml = ml_y, tl = lambdal_y,
                            cl = nul_y, Dl = Wl_y)
    
    elbo12 = elbo_q_THETA_X(mk = mk_x, tk = lambdak_x,
                            ck = nuk_x, Dk = Wk_x)
